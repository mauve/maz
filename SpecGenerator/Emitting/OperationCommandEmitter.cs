...
public static class OperationCommandEmitter
{
    ...
    )
    {
        ...
        var primaryPathParam = usesResourceGroup && !isMergedList
            ? op.CliParams
                .Where(p => p.ParamIn == "path")
                .OrderBy(p => op.UrlTemplate.IndexOf(
                    $"{{{p.UrlParameterName}}}", StringComparison.OrdinalIgnoreCase))
                .FirstOrDefault()
            : null;

        var resourceType = primaryPathParam is not null ? ExtractResourceType(op.UrlTemplate) : null;
            ...
            () =>
            {
                ...
                w.Block(
                    ...
                    () =>
                    {
                        ...
                        {
                            var primaryPropName = primaryPathParam!.CliFlag == "--format"
                                ? "ApiFormat"
