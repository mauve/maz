// <auto-generated/>
#nullable enable
#pragma warning disable CS1591
using System.Text.Json.Nodes;
using Console.Cli.Http;
using Console.Cli.Shared;
using Console.Rendering;

namespace Console.Cli.Commands.Generated;

/// <summary>List model versions.</summary>
public partial class MachinelearningModelversionListCommandDef(AuthOptionPack auth) : CommandDef
{
    public override string Name => "list";

    public readonly ResourceGroupOptionPack ResourceGroup = new();
    public readonly RenderOptionPack Render = new();

    /// <summary>Name of Azure Machine Learning workspace.</summary>
    [CliOption("--workspace-name", Required = true)]
    public partial string? WorkspaceName { get; }

    /// <summary>Container name. This is case-sensitive.</summary>
    [CliOption("--name", Required = true)]
    public partial string? ParamName { get; }

    /// <summary>Continuation token for pagination.</summary>
    [CliOption("--skip")]
    public partial string? Skip { get; }

    /// <summary>Ordering of list.</summary>
    [CliOption("--order-by")]
    public partial string? OrderBy { get; }

    /// <summary>Maximum number of records to return.</summary>
    [CliOption("--top")]
    public partial string? Top { get; }

    /// <summary>Model version.</summary>
    [CliOption("--version")]
    public partial string? Version { get; }

    /// <summary>Model description.</summary>
    [CliOption("--description")]
    public partial string? ParamDescription { get; }

    /// <summary>Number of initial results to skip.</summary>
    [CliOption("--offset")]
    public partial string? Offset { get; }

    /// <summary>Comma-separated list of tag names (and optionally values). Example: tag1,tag2=value2</summary>
    [CliOption("--tags")]
    public partial string? Tags { get; }

    /// <summary>Comma-separated list of property names (and optionally values). Example: prop1,prop2=value2</summary>
    [CliOption("--properties")]
    public partial string? Properties { get; }

    /// <summary>Name of the feed.</summary>
    [CliOption("--feed")]
    public partial string? Feed { get; }

    /// <summary>View type for including/excluding (for example) archived entities.</summary>
    [CliOption("--list-view-type")]
    public partial string? ListViewType { get; }

    private readonly AuthOptionPack _auth = auth;

    protected override async Task<int> ExecuteAsync(CancellationToken ct)
    {
        var client = new AzureRestClient(_auth.GetCredential());
        var path = $"/subscriptions/{ResourceGroup.Subscription.RequireSubscriptionId()}/resourceGroups/{ResourceGroup.RequireResourceGroupName()}/providers/Microsoft.MachineLearningServices/workspaces/{WorkspaceName}/models/{ParamName}/versions";

        var allItems = client.GetAllAsync(path, "2025-12-01", "value", "nextLink", ct);
        var renderer = Render.GetRendererFactory().CreateCollectionRenderer<System.Text.Json.Nodes.JsonNode>();
        await renderer.RenderAllAsync(System.Console.Out, allItems, ct);
        return 0;
    }
}
