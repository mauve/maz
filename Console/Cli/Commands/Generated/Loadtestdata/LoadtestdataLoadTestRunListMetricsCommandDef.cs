// <auto-generated/>
#nullable enable
#pragma warning disable CS1591
using System.Text.Json.Nodes;
using Console.Cli.Http;
using Console.Cli.Shared;
using Console.Rendering;
using Azure.ResourceManager;

namespace Console.Cli.Commands.Generated;

/// <summary>List the metric values for a load test run.</summary>
public partial class LoadtestdataLoadTestRunListMetricsCommandDef(AuthOptionPack auth) : CommandDef
{
    public override string Name => "list-metrics";
    protected override bool IsDataPlane => true;
    protected override bool IsDestructive => true;

    [CliOption("--loadtest-endpoint")]
    public partial string? LoadtestEndpoint { get; }

    public readonly DirectUriOptionPack LoadTestEndpoint = new();

    public readonly RenderOptionPack Render = new();

    /// <summary>Unique name for the load test run, must contain only lower-case alphabetic, numeric, underscore or hyphen characters.</summary>
    [CliOption("--test-run-id", Required = true)]
    public partial string? TestRunId { get; }

    /// <summary>The aggregation</summary>
    [CliOption("--aggregation")]
    public partial string? Aggregation { get; }

    /// <summary>Metric name</summary>
    [CliOption("--metricname", Required = true)]
    public partial string? Metricname { get; }

    /// <summary>The interval (i.e. timegrain) of the query.</summary>
    [CliOption("--interval")]
    public partial string? Interval { get; }

    /// <summary>Metric namespace to query metric definitions for.</summary>
    [CliOption("--metric-namespace", Required = true)]
    public partial string? MetricNamespace { get; }

    /// <summary>The timespan of the query. It is a string with the following format 'startDateTime_ISO/endDateTime_ISO'.</summary>
    [CliOption("--timespan", Required = true)]
    public partial string? Timespan { get; }

    private readonly AuthOptionPack _auth = auth;

    protected override async Task<int> ExecuteAsync(CancellationToken ct)
    {
        var client = new AzureRestClient(_auth.GetCredential(), "https://cnt-prod.loadtesting.azure.com/.default");
        var loadTestEndpointBaseUrl = LoadtestEndpoint ?? (await LoadTestEndpoint.ResolveDataplaneRefAsync(new ArmClient(_auth.GetCredential()), ct)).ToString().TrimEnd('/');
        var path = $"{loadTestEndpointBaseUrl}/test-runs/{TestRunId}/metrics";

        var allItems = client.GetAllAsync(path, "2022-11-01", "value", "nextLink", ct);
        var renderer = Render.GetRendererFactory().CreateCollectionRenderer<System.Text.Json.Nodes.JsonNode>();
        await renderer.RenderAllAsync(System.Console.Out, allItems, ct);
        return 0;
    }
}
