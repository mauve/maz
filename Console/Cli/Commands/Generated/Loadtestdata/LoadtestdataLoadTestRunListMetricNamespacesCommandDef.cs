// <auto-generated/>
#nullable enable
#pragma warning disable CS1591
using System.Text.Json.Nodes;
using Console.Cli.Http;
using Console.Cli.Shared;
using Console.Rendering;
using Azure.ResourceManager;

namespace Console.Cli.Commands.Generated;

/// <summary>List the metric namespaces for a load test run.</summary>
public partial class LoadtestdataLoadTestRunListMetricNamespacesCommandDef(AuthOptionPack auth) : CommandDef
{
    public override string Name => "list-metric-namespaces";
    protected override bool IsDataPlane => true;

    [CliOption("--loadtest-endpoint")]
    public partial string? LoadtestEndpoint { get; }

    public readonly DirectUriOptionPack LoadTestEndpoint = new();

    public readonly RenderOptionPack Render = new();

    /// <summary>Unique name for the load test run, must contain only lower-case alphabetic, numeric, underscore or hyphen characters.</summary>
    [CliOption("--test-run-id", Required = true)]
    public partial string? TestRunId { get; }

    private readonly AuthOptionPack _auth = auth;

    protected override async Task<int> ExecuteAsync(CancellationToken ct)
    {
        var client = new AzureRestClient(_auth.GetCredential(), "https://cnt-prod.loadtesting.azure.com/.default");
        var loadTestEndpointBaseUrl = LoadtestEndpoint ?? (await LoadTestEndpoint.ResolveDataplaneRefAsync(new ArmClient(_auth.GetCredential()), ct)).ToString().TrimEnd('/');
        var path = $"{loadTestEndpointBaseUrl}/test-runs/{TestRunId}/metric-namespaces";

        var result = await client.SendAsync(HttpMethod.Get, path, "2022-11-01", null, ct);
        await Render.GetRendererFactory().CreateRendererForType(typeof(System.Text.Json.Nodes.JsonNode))
            .RenderAsync(System.Console.Out, result, ct);
        return 0;
    }
}
