// <auto-generated/>
#nullable enable
#pragma warning disable CS1591
using System.Text.Json.Nodes;
using Console.Cli.Http;
using Console.Cli.Shared;
using Console.Rendering;
using Azure.ResourceManager;

namespace Console.Cli.Commands.Generated;

/// <summary>Gets a virtual machine image.</summary>
/// <remarks>Gets a virtual machine image.</remarks>
public partial class ComputeVirtualmachineimageShowCommandDef(AuthOptionPack auth) : CommandDef
{
    public override string Name => "show";

    public readonly SubscriptionOptionPack Subscription = new();
    public readonly RenderOptionPack Render = new();

    /// <summary>The name of Azure region.</summary>
    [CliOption("--location", Required = true)]
    public partial string? Location { get; }

    /// <summary>A valid image publisher.</summary>
    [CliOption("--publisher-name", Required = true)]
    public partial string? PublisherName { get; }

    /// <summary>A valid image publisher offer.</summary>
    [CliOption("--offer", Required = true)]
    public partial string? Offer { get; }

    /// <summary>A valid image SKU.</summary>
    [CliOption("--skus", Required = true)]
    public partial string? Skus { get; }

    /// <summary>A valid image SKU version.</summary>
    [CliOption("--version", Required = true)]
    public partial string? Version { get; }

    private readonly AuthOptionPack _auth = auth;

    protected override async Task<int> ExecuteAsync(CancellationToken ct)
    {
        var log = DiagnosticOptionPack.GetLog();
        var cred = _auth.GetCredential(log);
        var client = new AzureRestClient(cred, log);
        var subscriptionId = await Subscription.RequireSubscriptionIdAsync(new ArmClient(cred));
        var path = $"/subscriptions/{subscriptionId}/providers/Microsoft.Compute/locations/{Location}/publishers/{PublisherName}/artifacttypes/vmimage/offers/{Offer}/skus/{Skus}/versions/{Version}";

        var result = await client.SendAsync(HttpMethod.Get, path, "2025-04-01", null, ct);
        await Render.GetRendererFactory().CreateRendererForType(typeof(System.Text.Json.Nodes.JsonNode))
            .RenderAsync(System.Console.Out, result, ct);
        return 0;
    }
}
