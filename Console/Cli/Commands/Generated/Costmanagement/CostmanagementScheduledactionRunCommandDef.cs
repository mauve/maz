// <auto-generated/>
#nullable enable
#pragma warning disable CS1591
using System.Text.Json.Nodes;
using Console.Cli.Http;
using Console.Cli.Shared;
using Console.Rendering;

namespace Console.Cli.Commands.Generated;

/// <summary>Runs a shared scheduled action within the given scope.</summary>
/// <remarks>Runs a shared scheduled action within the given scope.</remarks>
public partial class CostmanagementScheduledactionRunCommandDef(AuthOptionPack auth) : CommandDef
{
    public override string Name => "run";
    protected override bool IsDestructive => true;

    public readonly RenderOptionPack Render = new();

    /// <summary>undefined</summary>
    [CliOption("--scope", Required = true)]
    public partial string? Scope { get; }

    /// <summary>Scheduled action name.</summary>
    [CliOption("--name", Required = true)]
    public partial string? ParamName { get; }

    private readonly AuthOptionPack _auth = auth;

    protected override async Task<int> ExecuteAsync(CancellationToken ct)
    {
        var client = new AzureRestClient(_auth.GetCredential());
        var path = $"/{Scope}/providers/Microsoft.CostManagement/scheduledActions/{ParamName}/execute";

        var result = await client.SendAsync(HttpMethod.Post, path, "2025-03-01", null, ct);
        await Render.GetRendererFactory().CreateRendererForType(typeof(System.Text.Json.Nodes.JsonNode))
            .RenderAsync(System.Console.Out, result, ct);
        return 0;
    }
}
