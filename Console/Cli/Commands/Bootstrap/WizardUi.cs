...
internal static class WizardUi
{
    public static int GetTermWidth()
    {
        try
        {
            return Math.Clamp(System.Console.WindowWidth, 40, 300);
        }
        catch
        {
            return 80;
    ...
    ///   ╔═  {title}  (n/total)  ══════════════╗
    /// </summary>
    public static void RenderTopBorder(
        string title,
        int stepIndex,
        int total,
        int boxWidth,
    ...
    public static void MoveTo(int row, int col = 1) => System.Console.Write($"\x1b[{row};{col}H");

    public static int GetTermHeight()
    {
        try
        {
            return Math.Clamp(System.Console.WindowHeight, 10, 500);
        }
        catch
        {
            return 24;
