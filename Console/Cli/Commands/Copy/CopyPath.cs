...
)
{
    ...
    public static CopyPath Parse(string raw)
    {
        ...
        // Shorthand: account/container[/prefix] — requires at least one /
        var parts = raw.Split('/', 3);
        if (parts.Length < 2)
        {
            // No slash → treat as local relative path (e.g. "smurf", "data")
            return new CopyPath(CopyPathKind.Local, null, null, null, raw, null);
        }
