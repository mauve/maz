    ...
    : CommandDef
{
    ...
    public readonly CliArgument<string> Source = new()
    {
        Name = "source",
        Description = "Source path (local, shorthand, or blob URL).",
    };

    public readonly CliArgument<string> Destination = new()
    {
        Name = "destination",
        Description = "Destination path (local, shorthand, or blob URL).",
    };
    ...
    protected override async Task<int> ExecuteAsync(CancellationToken ct)
    {
        ...
        var sourceRaw = GetValue(Source);
        var destRaw = GetValue(Destination);

        if (string.IsNullOrWhiteSpace(sourceRaw) || string.IsNullOrWhiteSpace(destRaw))
            throw new InvocationException("Both <source> and <destination> are required.");

        log.Trace($"Source: {sourceRaw}");
        log.Trace($"Destination: {destRaw}");

        var source = CopyPath.Parse(sourceRaw);
        var dest = CopyPath.Parse(destRaw);

        log.Trace($"Source kind: {source.Kind}, account: {source.AccountName}, container: {source.ContainerName}, prefix: {source.BlobPrefix}");
        log.Trace($"Dest kind: {dest.Kind}, account: {dest.AccountName}, container: {dest.ContainerName}, prefix: {dest.BlobPrefix}");

        // Validate at least one side is blob storage
        if (source.Kind == CopyPathKind.Local && dest.Kind == CopyPathKind.Local)
            throw new InvocationException(
                "At least one side of the copy must be Azure Blob Storage. Use cp/rsync for local-to-local."
            );

        // Build auth strategy
        var blobAuth = BuildAuthStrategy(source, dest, log);
        ...
        System.Console.Error.WriteLine("Enumerating source items...");
        var items = await plan.BuildAsync(ForceClientSide, ct);

        if (items.Count == 0)
        {
            System.Console.Error.WriteLine("No items to transfer.");
            return 0;
        }

        System.Console.Error.WriteLine($"Found {items.Count} item(s) to transfer.");

        // Journal for resume
        CopyJournal? journal = null;
        if (!NoJournal)
        {
            journal = CopyJournal.Open(sourceRaw, destRaw, JournalPath);
            journal.WritePlan(sourceRaw, destRaw, items.Count);

            // Filter out already-completed items
            var remaining = new List<TransferItem>();
            for (int i = 0; i < items.Count; i++)
            {
                if (!journal.IsItemCompleted(i))
                    remaining.Add(items[i]);
            }

            if (remaining.Count < items.Count)
                System.Console.Error.WriteLine(
                    $"Resuming: {items.Count - remaining.Count} item(s) already completed."
                );

            items = remaining;
            if (items.Count == 0)
            {
                System.Console.Error.WriteLine("All items already transferred.");
                journal.Cleanup();
                return 0;
            }
        }
        ...
        if (isInteractive)
        {
            var states = new CopyTransferState[items.Count];
            for (int i = 0; i < items.Count; i++)
                states[i] = new CopyTransferState(
                    items[i],
                    new CancellationTokenSource()
                );

            await using var tui = new CopyTuiApp(items, states, engine.Progress, engine);
            await tui.RunAsync(ct);
        }
        else
        {
            var sw = Stopwatch.StartNew();
            await CopyNdjsonOutput.DrainAsync(engine.Progress, items, sw, ct);
        }
    ...
    private IBlobAuthStrategy BuildAuthStrategy(CopyPath source, CopyPath dest, DiagnosticLog log)
    {
        ...
        var account = source.Kind == CopyPathKind.BlobStorage
            ? source.AccountName!
            : dest.AccountName!;
    ...
    private string? BuildTagQuery()
    {
        ...
        var parts = TagFilter.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
